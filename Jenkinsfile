        ...
        stage('Docker Build & Push') {
            steps {
                script {
                    ...
                    sh "echo $MY_PASS | sudo -S docker login -u $DOCKER_REGISTRY_USERNAME -p $DOCKER_REGISTRY_PASSWORD $DOCKER_REGISTRY_URL"

                    // Build and push Docker image
                    sh "echo $MY_PASS | sudo -S docker build -t $DOCKER_REGISTRY_URL/demo-spring-boot-app:latest ."
                    sh "echo $MY_PASS | sudo -S docker push $DOCKER_REGISTRY_URL/demo-spring-boot-app:latest"
                }
            }
        }
